...
if ($searchKeyword == null) {       // 검색을 안한 경우
    ...
    else { ?> <a href="boardController.php?nowPage=<?php echo $startPageNum-1; ?>"><<</a> <?php }
    // < 기능
    if ($clickPageButton <= 1){     // 출력된 블록 안의 클릭된 버튼이 1인 경우, < 클릭 불가
        echo "<span><</span>";
    } else{                         // 출렫된 블록 안의 클릭된 버튼이 1이 아닌 경우, < 클릭 가능
        ?> <a href="boardController.php?nowPage=<?php echo $clickPageButton-1; ?>"><</a> <?php
    }
    // 버튼 출력
    for ($num = $startPageNum; $num <= $endPageNum; $num++){
        if ($num == $clickPageButton){      // 출력된 블록 안의 클릭된 숫자의 경우, 클릭 불가
            ...
            ?> <a href="boardController.php?nowPage=<?php echo $num; ?>"><?php echo $num; ?></a> <?php
        }
    }
    // > 기능
    ...
    else { ?> <a href="boardController.php?nowPage=<?php echo $endPageNum+1; ?>">>></a> <? }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
else {                              // 검색을 한 경우
    // << 기능
    if ($clickPageButton <= $showButtonNum){ echo "<span><<</span>"; }
        ...
        ?> <a href="boardController.php?nowPage=<?php echo $searchKeyword; ?>&searchText=<?php echo $searchText; ?>&searchBtn=검색&nowPage=<?php echo $startPageNum-1; ?>"><<</a> <?
    }
    // < 기능
    if ($clickPageButton <= 1){     // 출력된 블록 안의 클릭된 버튼이 1인 경우, < 클릭 불가
        echo "<span><</span>";
    } else{                         // 출렫된 블록 안의 클릭된 버튼이 1이 아닌 경우, < 클릭 가능
        ?> <a href="boardController.php?keyword=<?php echo $searchKeyword; ?>&searchText=<?php echo $searchText; ?>&searchBtn=검색&nowPage=<?php echo $clickPageButton-1; ?>"><</a> <?php
    }
    // 버튼 출력
    for ($num = $startPageNum; $num <= $endPageNum; $num++){
        if ($num == $clickPageButton){      // 출력된 블록 안의 클릭된 숫자의 경우, 클릭 불가
            ...
            ?> <a href="boardController.php?keyword=<?php echo $searchKeyword; ?>&searchText=<?php echo $searchText; ?>&searchBtn=검색&nowPage=<?php echo $num; ?>"><?php echo $num; ?></a> <?php
        }
    }
    // > 기능
    if ($clickPageButton >= $totalPageNum){     // 출력된 블록 안의 클릭된 버튼이 버튼의 마지막 값인 경우, > 클릭 불가
    ...
    if ($endPageNum >= $totalPageNum){
        ...
        ?> <a href="boardController.php?keyword=<?php echo $searchKeyword; ?>&searchText=<?php echo $searchText; ?>&searchBtn=검색&nowPage=<?php echo $endPageNum+1; ?>">>></a> <?
    }
}
// button 기능 <<--
