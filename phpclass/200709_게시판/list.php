        ...
        if ($searchKeyword == null){    // 검색 안한 경우
            ...
            if ($searchKeyword == "제목_내용") {    // 제목 + 내용으로 검색한 경우
                $sql       = "select * from mybulletin where title like '%{$searchText}%' or contents like '%{$searchText}%' and board_pid=0 
                              order by board_id desc limit {$nowPage},{$showTextNum}";
                $changeSql = "select * from mybulletin where title like '%{$searchText}%' or contents like '%{$searchText}%' and board_pid=0 
                              order by board_id";
...
<div>
    ...
    if ($searchKeyword == null) {       // 검색을 안한 경우
        ...
        if ($clickPageButton <= 1){     // 출력된 블록 안의 클릭된 버튼이 1인 경우, << 클릭 불가
            ...
            ?>
            <a href="<?php echo $_SERVER[PHP_SELF]; ?>?keyword=<?php echo $searchKeyword;?>&searchText=<?php echo $searchText; ?>
