...
if (password_verify($userInputPasswd,$userPasswd)){ // 패스워드 확인
    ...
    message("덧글이 성공적으로 삭제되었습니다.",BoardInfo::FILENAME_VIEW."?board_id={$board_id}&nowPage={$nowPage}");
} else {
    // 비밀번호 불일치, view.php 로 이동
    message("비밀번호를 확인해 주세요.",BoardInfo::FILENAME_VIEW."?board_id={$board_id}&nowPage={$nowPage}");
}
