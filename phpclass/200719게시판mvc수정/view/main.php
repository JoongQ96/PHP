...
<header>
    <table class="myTable">
        <tr>
            <td><a href="<?php echo $_SERVER[PHP_SELF];?>"><img src="home.png" alt="홈으로"></a></td>
...
<div class="container">
    <nav id="content1">
        ...
        <?php if(isset($_SESSION['id'])): // 로그인 한 경우에 버튼 출력 ?>
            <button style="width: 100%" onclick="location.href='../controller/writeController.php'">글쓰기</button>
        <?php endif; ?>
    </nav>
    <article id="content2">
        <h2>Main Content</h2>
        ...
        <!------------------------------------------ 검 색 ---------------------------------------------------------------------->
        <form action="../controller/boardController.php" method="get">
            <select name="keyword">
        ...
        <?php if ($searchKeyword != null): // 검색을 한 경우에 버튼 출력 ?>
            <button onclick="location.href='../controller/boardController.php'">리스트</button>
        <?php endif; ?>
    </article>
</div>
</body>
