<fieldset style="width: 50%">
    ...
    <!-- main.php 파일에서 선택한 게시글 출력 -->
    <form action="<?php echo BoardInfo::FILENAME_MODIFY; ?>" method="post">
        <table>
                ...
                <td colspan="2">
                    ...
                    if ($searchKeyword != null){    // list에서 검색 한 경우
                        echo "<input type='button' name='list' value='글 목록' 
                              ...
                              'main?board_id={$boardValue->board_id}&keyword={$searchKeyword}&searchText={$searchText}&searchBtn={$searchBtn}&nowPage={$nowPage}'\">";
                    } else{
                        if ($nowPage != null){      // list에서 검색 안한 경우, nowPage 있을때
                            echo "<input type='button' name='list' value='글 목록' onclick=\"location.href='main?nowPage={$nowPage}'\">";
                        } else{                     // list에서 검색 안한 경우, nowPage 없을때, 즉 default page
                            echo "<input type='button' name='list' value='글 목록' onclick=\"location.href='main?nowPage=1'\">";
                        }
                    }
                    ?>
                    ...
                    <?php if ($_SESSION['id'] == $boardValue->user_name): // 글 작성자인 경우만 수정, 삭제 가능 ?>
    ...
    <?php if (isset($_SESSION['id'])): // login 한 경우 ?>
        ...
        <form action="<?php echo BoardInfo::FILENAME_WRITE_PROCESS; ?>" method="post">
            <table>
                <tr><td>댓글</td></tr>
    ...
    <form action="<?php echo BoardInfo::FILENAME_DELETE_PROCESS; ?>" method="post">
        <table>
            ...
            <?php $obj->showComment($boardValue->board_id); // 덧글 출력 기능 (게시글 번호, 현재 페이지 번호) ?>
        </table>
    </form>
</fieldset>
