...
@section('content')
    ...
    <div class="px-64">
        ...
        <form action="/writers" method="POST">
            ...
            <textarea class="border board-gray-800 w-full @error('body') border border-red-700 @enderror" name="body" id="body" cols="30" rows="10" required>{{ old('title') ? old('title') : '' }}</textarea><br>
            @error('body')
            <small class="text-red-700">{{ $message }}</small>
            @enderror
